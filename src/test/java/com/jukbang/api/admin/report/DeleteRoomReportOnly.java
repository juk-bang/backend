...
public class DeleteRoomReportOnly extends BaseControllerTest {
    @Autowired
    RoomReportService roomReportService ;

    @Test
    @DisplayName("신고한 방 잔류하기 (성공)")
    void deleteReportRoomSuccess() throws Exception{
        ...
        this.mockMvc.perform(RestDocumentationRequestBuilders.delete("/admin/report/rooms/{reportid}",roomReportId)
                .header("Authorization", "Bearer " + accessToken))
                .andExpect(status().isOk())
                .andDo(print())
                .andDo(document("deleteRoomReportOnly"))
        ;
